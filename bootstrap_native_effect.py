...
def commpute_differences(filename_, levels, values_comparison_french, values_comparison_english, sampled_lines):
    ...
    for i in range(len(values_comparison_english)):
        val_fr = gf[values_comparison_french[i]].mean()
        ans_fr[values_comparison_french[i]] = val_fr[values_comparison_french[i]]

    gf = data_en.groupby(['filename', 'TGT', 'OTH', 'prev_phone', 'next_phone', 'language_stimuli'], as_index=False)
    ans_en = gf.correct_answer.mean()
    for i in range(len(values_comparison_english)):
        val_en = gf[values_comparison_english[i]].mean()
        ans_en[values_comparison_english[i]] = val_en[values_comparison_english[i]]
    ...
    if  'file' in levels:
        ...
        for trip in triplet_list:
            ...
            for i in range(len(values_comparison_english)):
                val_fr_model = ans_fr[ans_fr['filename'] == trip][values_comparison_french[i]].iloc[0]
                val_en_model = ans_en[ans_en['filename'] == trip][values_comparison_english[i]].iloc[0]
    ...
    if 'triplet' in levels:
        ans_en['triplet_compo'] = ans_en['TGT'] + ',' + ans_en['OTH'] + ',' + ans_en['prev_phone'] + ',' + ans_en['next_phone'] + ',' + ans_en['language_stimuli']
        ans_fr['triplet_compo'] = ans_fr['TGT'] + ',' + ans_fr['OTH'] + ',' + ans_fr['prev_phone'] + ',' + ans_fr[
            'next_phone'] + ',' + ans_fr['language_stimuli']
        triplet_list = list(ans_en.triplet_compo.unique())
        diff_humans = []
        diff_models = {}
        for i in range(len(values_comparison_english)):
            diff_models[values_comparison_english[i]] = []
        ...
        for trip in triplet_list:
            ...
            val_fr_human = (ans_fr[ans_fr['triplet_compo'] == trip].correct_answer.iloc[0] + ans_fr[ans_fr['triplet_compo'] == other].correct_answer.iloc[0])/2.
            val_en_human = (ans_en[ans_en['triplet_compo'] == trip].correct_answer.iloc[0] + ans_en[ans_en['triplet_compo'] == other].correct_answer.iloc[0])/2.
            diff_humans.append(val_fr_human - val_en_human)
            for i in range(len(values_comparison_english)):
                val_fr_model = (ans_fr[ans_fr['triplet_compo'] == trip][values_comparison_french[i]].iloc[0] +
                                ans_fr[ans_fr['triplet_compo'] == other][values_comparison_french[i]].iloc[0])/2.
                val_en_model = (ans_en[ans_en['triplet_compo'] == trip][values_comparison_english[i]].iloc[0] +
                                ans_en[ans_en['triplet_compo'] == other][values_comparison_english[i]].iloc[0])/2.
                diff_models[values_comparison_english[i]].append(val_fr_model - val_en_model)
        all_val += [np.asarray(diff_humans)]
        for i in range(len(values_comparison_english)):
            all_val += [diff_models[values_comparison_english[i]]]
    ...
    if  'contrast' in levels:
        ...
        for trip in triplet_list:
            ...
            diff_humans.append(val_fr_human - val_en_human)
            for i in range(len(values_comparison_english)):
                val_fr_model = (ans_fr[ans_fr['contrast'] == trip][values_comparison_french[i]].iloc[0] +
                                ans_fr[ans_fr['contrast'] == other][values_comparison_french[i]].iloc[0]) / 2.
                val_en_model = (ans_en[ans_en['contrast'] == trip][values_comparison_english[i]].iloc[0] +
                                ans_en[ans_en['contrast'] == other][values_comparison_english[i]].iloc[0]) / 2.
...
def commpute_differences_faster(filename_, levels, values_comparison_french, values_comparison_english, sampled_lines, it):
    ...
    for k in values_comparison_french  + ['binarized_answer']:
        data_fr_file[k] = {}
        data_fr_triplet[k] = {}
        data_fr_contrast[k] = {}
    for k in values_comparison_english  + ['binarized_answer']:
        data_en_file[k] = {}
        data_en_triplet[k] = {}
        data_en_contrast[k] = {}
    ...
    f.close()
    #print(data_fr_contrast["binarized_answer"].keys())
    ...
    if  'file' in levels:
        ...
        for trip in triplet_list:
            ...
            triplet_done.append(other)
            # we average OTH-TGT and TGT-OTH
            val_fr_human = (data_fr_file['binarized_answer'][trip] + data_fr_file['binarized_answer'][other])/2.
            val_en_human = (data_en_file['binarized_answer'][trip] + data_en_file['binarized_answer'][other])/2.
            # it is already averaged for the model
    ...
    if 'triplet' in levels:
        ...
        for trip in triplet_list:
            ...
            val_fr_human = (data_fr_triplet['binarized_answer'][trip] + data_fr_triplet['binarized_answer'][other]) / 2.
            val_en_human = (data_en_triplet['binarized_answer'][trip] + data_en_triplet['binarized_answer'][other]) / 2.
            diff_humans.append(val_fr_human - val_en_human)
            for i in range(len(values_comparison_english)):
    ...
    if  'contrast' in levels:
        ...
        for trip in triplet_list:
            ...
            val_fr_human = (data_fr_contrast['binarized_answer'][trip] + data_fr_contrast['binarized_answer'][other])  / 2.
            val_en_human = (data_en_contrast['binarized_answer'][trip] + data_en_contrast['binarized_answer'][other])  / 2.
            diff_humans.append(val_fr_human - val_en_human)
            for i in range(len(values_comparison_english)):
...
if __name__ == '__main__':
    ...
    dico_models = {'wav2vec_10k_transf4_ft':{'english':'wav2vec_10k_en_transf4', 'french':'wav2vec_10k_fr_transf4'},
                   'overlap score':{'english':'overlap_score_naive', 'french':'overlap_score_naive'},
